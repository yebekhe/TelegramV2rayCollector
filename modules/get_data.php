...
function generate_name($input, $type, $channel, $flag, $ping, $is_reality){
    switch ($type){
        ...
        case "trojan":
            ...
            break;
        case "ss":
            $input["remarks"] = "@" . $channel . " | " . $flag . " | " . $ping;
            break;
    }
    return $input;
}
...
function parse_config($input, $type){
    switch ($type){
        case "vmess":
            ...
            break;
        case "vless":
            $parsed_config = parseProxyUrl($type . "://" . $input, "vless");
            break;
        case "trojan":
            $parsed_config = parseProxyUrl($type . "://" . $input);
            break;
        case "ss":
            $parsed_config = ParseShadowsocks($type . "://" . $input);
            break;
    }
